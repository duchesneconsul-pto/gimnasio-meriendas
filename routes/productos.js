...
router.post('/', verificarToken, soloAdmin, (req, res) => {
  const { nombre, categoria, precio_compra, precio_venta, stock_actual, stock_minimo } = req.body;
  ...
  ).run(nombre, categoria || 'general', precio_compra || 0, precio_venta, stock_actual || 0, stock_minimo || 5);
  db._save();
...
router.put('/:id', verificarToken, soloAdmin, (req, res) => {
  ...
  db.prepare(
    'UPDATE productos SET nombre=?, categoria=?, precio_compra=?, precio_venta=?, stock_minimo=?, activo=? WHERE id=?'
  ).run(
